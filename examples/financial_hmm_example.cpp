...
int main() {
    ...
    Matrix trans(2, 2);
    trans(0, 0) = 0.85;
    trans(0, 1) = 0.15; // Low vol tends to persist
    trans(1, 0) = 0.25;
    trans(1, 1) = 0.75; // High vol is somewhat persistent
    hmm->setTrans(trans);
    ...
    std::cout << "P(vol=0.1 | Low regime)  = " << hmm->getDistribution(0).getProbability(0.1)
              << std::endl;
    std::cout << "P(vol=0.1 | High regime) = " << hmm->getDistribution(1).getProbability(0.1)
              << std::endl;
    std::cout << "P(vol=0.5 | Low regime)  = " << hmm->getDistribution(0).getProbability(0.5)
              << std::endl;
    std::cout << "P(vol=0.5 | High regime) = " << hmm->getDistribution(1).getProbability(0.5)
              << std::endl;
    std::cout << std::endl;
