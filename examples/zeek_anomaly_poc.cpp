...
static std::vector<Sequence> load_sequences(const std::string& path)
{
    ...
    while (std::getline(f, line)) {
        ...
        if (!std::getline(ss, f0s,  ',')) continue;
        if (!std::getline(ss, f1s,  ',')) continue;
        if (!std::getline(ss, f2s,  ',')) continue;
        if (!std::getline(ss, f3s       )) continue;
        const int idx = std::stoi(idx_s);
        if (idx == 0 && !cur.key.empty()) {
            seqs.push_back(std::move(cur));
            cur = {};
        }
        if (idx == 0) cur.key = key;
        cur.obs.push_back({std::stod(f0s), std::stod(f1s),
                           std::stod(f2s), std::stod(f3s)});
    }
    if (!cur.key.empty()) seqs.push_back(std::move(cur));
    return seqs;
}
...
static double run_bwt(HmmMV& hmm, const MultiObservationLists& lists)
{
    ...
    for (int k = 0; k < MAX_ITER; ++k) {
        ...
        if (k > 0 && delta >= -1e-8 && delta < CONV_TOL) {
            if (conv_iter < 0) conv_iter = k;
        }
        if (conv_iter >= 0 && k >= conv_iter + 2) break;
        prev = cur;
    }
...
static HmmMV make_hmm()
{
    ...
    hmm.setTrans(A);
    Vector pi(N_STATES);
    for (int i = 0; i < N_STATES; ++i) pi(i) = 1.0 / N_STATES;
    hmm.setPi(pi);
    return hmm;
}
...
static std::vector<ScoredSeq> score_all(
    HmmMV& hmm,
    ...
    const std::unordered_map<std::string, Label>& labels)
{
    ...
    for (const auto& s : seqs) {
        ...
        if (it != labels.end()) bot = it->second.is_botnet;
        result.push_back({s.key, norm, bot});
    }
    return result;
}

static void print_detection_table(const std::vector<ScoredSeq>& scored,
                                   double benign_mean, double benign_sd)
{
    ...
    for (double k : {1.0, 1.5, 2.0, 2.5, 3.0}) {
        ...
        for (const auto& s : scored) {
            ...
            if (s.is_botnet) { if (flagged) ++tp; else ++fn; }
        ...
        std::cout << std::fixed << std::setprecision(1)
                  << std::setw(8)  << k
                  << std::setw(12) << std::setprecision(3) << thr
                  << std::setw(8)  << std::setprecision(1) << tpr
                  << std::setw(8)  << fpr
                  << "\n";
    }
}
...
int main(int argc, char* argv[])
{
    ...
    for (const auto& s : test_seqs) {
        auto it = labels.find(s.key);
        if (it != labels.end() && it->second.is_botnet) ++n_botnet_test;
        else ++n_benign_test;
    }
    const auto train_lists = to_obs_lists(train_seqs);

    std::cout << "  Training keys (benign only): " << n_train << "\n";
    std::cout << "  Test keys — benign: " << n_benign_test
              << "  botnet: " << n_botnet_test << "\n\n";
    ...
    for (const auto& s : scores_a) {
        if (s.is_botnet) { sum_m_a += s.norm_score; sum2_m_a += s.norm_score*s.norm_score; ++cnt_m_a; }
        else             { sum_b_a += s.norm_score; sum2_b_a += s.norm_score*s.norm_score; ++cnt_b_a; }
    }
    ...
    const double sep_a    = (mean_b_a - mean_m_a) / std::sqrt(0.5*(sd_b_a*sd_b_a + sd_m_a*sd_m_a));

    std::cout << std::fixed << std::setprecision(4)
              << "  Benign  mean score: " << mean_b_a << "  sd: " << sd_b_a << "\n"
    ...
    for (const auto& s : scores_b) {
        if (s.is_botnet) { sum_m_b += s.norm_score; sum2_m_b += s.norm_score*s.norm_score; ++cnt_m_b; }
        else             { sum_b_b += s.norm_score; sum2_b_b += s.norm_score*s.norm_score; ++cnt_b_b; }
    }
    ...
    const double sep_b    = (mean_b_b - mean_m_b) / std::sqrt(0.5*(sd_b_b*sd_b_b + sd_m_b*sd_m_b));

    std::cout << std::fixed << std::setprecision(4)
              << "  Benign  mean score: " << mean_b_b << "  sd: " << sd_b_b << "\n"
    ...
    std::cout << std::fixed << std::setprecision(2)
              << std::setw(30) << "Training LL"
              << std::setw(14) << ll_a
              << std::setw(14) << ll_b << "\n"
              << std::setw(30) << "BIC (lower = better)"
              << std::setw(14) << bic_a
              << std::setw(14) << bic_b << "\n"
              << std::setw(30) << "Free parameters k"
              << std::setw(14) << static_cast<long>(k_a)
              << std::setw(14) << static_cast<long>(k_b) << "\n"
              << std::setw(30) << "Cohen's d (separation)"
              << std::setw(14) << std::setprecision(3) << sep_a
              << std::setw(14) << sep_b << "\n"
              << std::setw(30) << "Wall time (ms)"
