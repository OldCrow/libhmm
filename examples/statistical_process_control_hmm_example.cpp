...
int main() {
    ...
    trans(0, 0) = 0.90;
    trans(0, 1) = 0.08;
    trans(0, 2) = 0.02; // Mostly stable
    // Warning transitions
    trans(1, 0) = 0.20;
    trans(1, 1) = 0.60;
    trans(1, 2) = 0.20; // Can improve or degrade
    // Out of Control transitions
    trans(2, 0) = 0.10;
    trans(2, 1) = 0.30;
    trans(2, 2) = 0.60; // Tends to persist until fixed
    hmm->setTrans(trans);
    ...
    for (int state = 0; state < 3; ++state) {
        const auto *dist =
            dynamic_cast<const ChiSquaredDistribution *>(&hmm->getDistribution(state));
        ...
        std::cout << "  Mode: "
                  << ((dist->getDegreesOfFreedom() >= 2) ? (dist->getDegreesOfFreedom() - 2) : 0)
                  << "\n";
        std::cout << std::endl;
    }
    ...
    for (size_t i = 0; i < testStatSequence.size(); ++i) {
        std::cout << std::setw(6) << i + 1 << " | " << std::setw(9) << std::setprecision(1)
                  << testStatSequence(i) << " | " << std::setw(16) << stateLabels[states(i)]
                  << "\n";
    }
    std::cout << std::endl;
    ...
    multivarHmm->setDistribution(
        0, std::make_unique<ChiSquaredDistribution>(3.0)); // 3 characteristics
    multivarHmm->setDistribution(
        1, std::make_unique<ChiSquaredDistribution>(5.0)); // 5 characteristics
    multivarHmm->setDistribution(
        2, std::make_unique<ChiSquaredDistribution>(10.0)); // 10 characteristics
    ...
    varianceHmm->setDistribution(0,
                                 std::make_unique<ChiSquaredDistribution>(6.0)); // Normal variance
    varianceHmm->setDistribution(
        1, std::make_unique<ChiSquaredDistribution>(10.0)); // Increased variance
    varianceHmm->setDistribution(2,
                                 std::make_unique<ChiSquaredDistribution>(16.0)); // High variance

    std::cout << "Variance Control Chart Configuration:\n";
    for (int state = 0; state < 3; ++state) {
        const auto *dist =
            dynamic_cast<const ChiSquaredDistribution *>(&varianceHmm->getDistribution(state));
    ...
    for (int state = 0; state < 3; ++state) {
        const auto *dist =
            dynamic_cast<const ChiSquaredDistribution *>(&trainHmm->getDistribution(state));
        std::vector<std::string> stateLabels = {"In Control", "Warning", "Out of Control"};
        std::cout << stateLabels[state] << ": df=" << std::setprecision(2)
                  << dist->getDegreesOfFreedom() << ", Mean=" << dist->getMean()
                  << ", Std=" << dist->getStandardDeviation() << "\n";
    }
    std::cout << std::endl;
