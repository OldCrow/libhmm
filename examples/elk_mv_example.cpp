                      ...
                      int max_iter = 200, double tol = 1e-4)
{
    ...
    for (int k = 0; k < max_iter; ++k) {
        ...
        if (k > 0 && delta >= -1e-8 && delta < tol) { if (conv < 0) conv = k; }
...
int main(int argc, char* argv[])
{
    ...
    {
        Matrix tr(2, 2);
        tr(0,0)=0.9; tr(0,1)=0.1; tr(1,0)=0.1; tr(1,1)=0.9;
        hmm.setTrans(tr);
    ...
    std::cout << std::setw(28) << " "
              << std::setw(14) << "libhmm v4"
              << std::setw(14) << "moveHMM\n";
    std::cout << std::string(56, '-') << "\n";
    std::cout << std::fixed << std::setprecision(4);

    const auto& ic0 = static_cast<const IndependentComponentsDistribution&>(hmm.getDistribution(0));
    const auto& ic1 = static_cast<const IndependentComponentsDistribution&>(hmm.getDistribution(1));
    ...
    struct Row { const char* name; double lib; double ref; };
    const Row rows[] = {
    ...
    std::cout << std::setw(28) << "Wall time"
              << std::setw(14) << wall_str
              << std::setw(14) << "~2000 ms\n";
