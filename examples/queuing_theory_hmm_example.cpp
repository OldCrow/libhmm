...
int main() {
    ...
    arrivalTrans(0, 0) = 0.75;
    arrivalTrans(0, 1) = 0.20;
    arrivalTrans(0, 2) = 0.05;
    // Medium Load: can go either way
    arrivalTrans(1, 0) = 0.15;
    arrivalTrans(1, 1) = 0.65;
    arrivalTrans(1, 2) = 0.20;
    // High Load: tends to persist, difficult to recover
    arrivalTrans(2, 0) = 0.05;
    arrivalTrans(2, 1) = 0.25;
    arrivalTrans(2, 2) = 0.70;
    ...
    serviceTrans(0, 0) = 0.90;
    serviceTrans(0, 1) = 0.10; // Efficient service tends to persist
    serviceTrans(1, 0) = 0.30;
    serviceTrans(1, 1) = 0.70; // Slow service can persist due to queue buildup
    ...
    double serviceTimes[] = {4.2,  3.8, 5.1, 2.9, 4.5, 8.7, 12.3, 9.8,
                             11.2, 6.5, 4.8, 3.7, 5.2, 4.1, 3.9};
    ...
    mgOneHmm->setDistribution(
        0, std::make_unique<GammaDistribution>(4.0, 1.25)); // Efficient, consistent
    ...
    std::cout << "Arrival Rates: Low=" << lambda_low << ", Medium=" << lambda_med
              << ", High=" << lambda_high << " customers/hour\n";
    std::cout << "Service Rates: Efficient=" << service_rate_efficient * 60
              << ", Slow=" << service_rate_slow * 60 << " customers/hour\n\n";

    // Traffic intensity (ρ = λ/μ)
    std::cout << "Traffic Intensity (ρ = λ/μ):\n";
    std::cout << "Low Load + Efficient:   ρ = " << lambda_low / (service_rate_efficient * 60)
              << std::endl;
    std::cout << "Medium Load + Efficient: ρ = " << lambda_med / (service_rate_efficient * 60)
              << std::endl;
    std::cout << "High Load + Efficient:  ρ = " << lambda_high / (service_rate_efficient * 60)
              << std::endl;
    std::cout << "Medium Load + Slow:     ρ = " << lambda_med / (service_rate_slow * 60)
              << std::endl;
    std::cout << "High Load + Slow:       ρ = " << lambda_high / (service_rate_slow * 60)
              << " (UNSTABLE!)" << std::endl;
    std::cout << std::endl;
