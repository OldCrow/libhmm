...
struct SwarmObservation {
    ...
    unsigned int toDiscreteValue() const {
        ...
        return altitude_zone * 81 + speed_category * 27 + threat_level * 9 + formation_quality * 3 +
               communication;
    }
...
public:
    ...
    DroneSwarmController()
        : randomGen_(std::chrono::steady_clock::now().time_since_epoch().count()) {
    ...
    void setupObservationModels() {
        ...
        std::cout << "✅ Observation models configured for all " << numObservations
     ...
     */
    void setupStateObservationProbabilities(FormationState state,
                                            libhmm::DiscreteDistribution &dist) {
        ...
        switch (state) {
            case FormationState::STANDBY:
                observations.push_back(
                    {0, 0, 0, 0, 2}); // Low alt, no speed, no threat, poor formation, good comms
                observations.push_back({0, 0, 0, 0, 3}); // Variant with excellent comms
                break;

            case FormationState::TAKEOFF:
                observations.push_back(
                    {0, 1, 0, 1, 2}); // Low->medium alt, medium speed, acceptable formation
            ...
            case FormationState::EMERGENCY:
                observations.push_back(
                    {0, 0, 3, 0, 0}); // Emergency landing, high threat, poor everything
        ...
        if (!typical.empty()) {
            ...
            if (std::uniform_real_distribution<>(0.0, 1.0)(randomGen_) < noiseLevel) {
                ...
                switch (dimension) {
                    case 0:
                        obs.altitude_zone =
                            std::min(2, std::max(0, obs.altitude_zone +
                                                        static_cast<int>(randomGen_() % 3) - 1));
                        break;
                    case 1:
                        obs.speed_category =
                            std::min(2, std::max(0, obs.speed_category +
                                                        static_cast<int>(randomGen_() % 3) - 1));
                        break;
                    case 2:
                        obs.threat_level =
                            std::min(3, std::max(0, obs.threat_level +
                                                        static_cast<int>(randomGen_() % 3) - 1));
                        break;
                    case 3:
                        obs.formation_quality =
                            std::min(3, std::max(0, obs.formation_quality +
                                                        static_cast<int>(randomGen_() % 3) - 1));
                        break;
                    case 4:
                        obs.communication =
                            std::min(3, std::max(0, obs.communication +
                                                        static_cast<int>(randomGen_() % 3) - 1));
                        break;
                }
            }
            ...
            obs = {static_cast<int>(randomGen_() % 3), static_cast<int>(randomGen_() % 3),
                   static_cast<int>(randomGen_() % 4), static_cast<int>(randomGen_() % 4),
    ...
    void runMissionSimulation() {
        ...
        std::vector<FormationState> missionPhases = {
            FormationState::STANDBY,        FormationState::TAKEOFF,
            FormationState::CRUISE,         FormationState::FORMATION_TIGHT,
            FormationState::SEARCH_PATTERN, FormationState::FORMATION_TIGHT,
            FormationState::RETURN_TO_BASE, FormationState::STANDBY};
        ...
        for (size_t step = 0; step < missionPhases.size() * stepsPerPhase; ++step) {
            ...
            FormationState actualState =
                missionPhases[std::min(step / stepsPerPhase, missionPhases.size() - 1)];
            ...
            std::cout << " | Confidence: " << std::setw(6) << std::fixed << std::setprecision(1)
                      << confidence << "%";
        ...
        for (size_t i = windowSize; i < stateHistory_.size(); ++i) {
            std::vector<SwarmObservation> window(observationHistory_.begin() + i - windowSize + 1,
    ...
    void demonstrateRealTimeMonitoring() {
        ...
        std::vector<FormationState> scenario = {
            FormationState::CRUISE,          FormationState::CRUISE,
