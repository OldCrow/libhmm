...
TEST_F(NumericalStabilityTest, ProbabilityValidation) {
    ...
    EXPECT_THROW(NumericalSafety::checkProbability(std::numeric_limits<double>::quiet_NaN()),
                 std::runtime_error);
}
...
TEST_F(NumericalStabilityTest, LogProbabilityValidation) {
    ...
    EXPECT_THROW(NumericalSafety::checkLogProbability(std::numeric_limits<double>::quiet_NaN()),
                 std::runtime_error);
}
...
TEST_F(NumericalStabilityTest, ErrorRecoveryOverflow) {
    ...
    EXPECT_TRUE(
        ErrorRecovery::recoverFromOverflow(values, ErrorRecovery::RecoveryStrategy::GRACEFUL));
...
TEST_F(NumericalStabilityTest, ErrorRecoveryNaNHandling) {
    ...
    std::size_t nanCount =
        ErrorRecovery::handleNaNValues(values, ErrorRecovery::RecoveryStrategy::GRACEFUL);
