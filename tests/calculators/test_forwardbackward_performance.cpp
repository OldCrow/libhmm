...
// Test function for the standard calculator
PerformanceResult testStandardCalculator(libhmm::Hmm* hmm, const libhmm::ObservationSet& obs, std::size_t iterations = 5) {
    ...
    // Warmup
    {
        libhmm::ForwardBackwardCalculator warmup_calc(hmm, obs);
        log_prob = std::log(warmup_calc.probability());
    }
    
    // Benchmark
    for (std::size_t i = 0; i < iterations; ++i) {
        timer.start();
        libhmm::ForwardBackwardCalculator calc(hmm, obs);
        volatile double prob = calc.probability();
...
// Test function for SIMD calculators (inheriting directly from Calculator)
template<typename SIMDCalculator>
PerformanceResult testSIMDCalculator(const std::string& name,
    ...
    // Warmup
    {
        SIMDCalculator warmup_calc(hmm, obs);
        warmup_calc.compute();
    ...
    for (std::size_t i = 0; i < iterations; ++i) {
        timer.start();
        SIMDCalculator calc(hmm, obs);
        calc.compute();
        volatile double prob = calc.getProbability();
        total_time += timer.stop();
        (void)prob;
    }
...
int main() {
    ...
    for (const auto& config : configs) {
        ...
        auto results = std::vector<PerformanceResult>{
            testStandardCalculator(hmm.get(), obs),
                
            testSIMDCalculator<libhmm::LogSIMDForwardBackwardCalculator>("Log-SIMD", hmm.get(), obs),
                
            testSIMDCalculator<libhmm::ScaledSIMDForwardBackwardCalculator>("Scaled-SIMD", hmm.get(), obs)
        };
