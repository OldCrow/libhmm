...
TEST_F(CalculatorTraitsTest, CalculatorCreation) {
    ...
    EXPECT_NO_THROW(standardAuto.decode());
    EXPECT_NO_THROW(standardAuto.getLogProbability());
    EXPECT_NO_THROW(standardAuto.getStateSequence());
...
TEST_F(CalculatorTraitsTest, OptimalCreation) {
    // Use AutoCalculator for Viterbi calculations
    AutoCalculator optimalAuto(smallHmm_.get(), shortObs_);
    const StateSequence seq1 = optimalAuto.decode();
    const double logProb1 = optimalAuto.getLogProbability();
    
    AutoCalculator standardAuto(smallHmm_.get(), shortObs_);
    const StateSequence seq2 = standardAuto.decode();
    const double logProb2 = standardAuto.getLogProbability();
    
    // Should produce reasonable results
    EXPECT_EQ(seq1.size(), shortObs_.size());
    EXPECT_EQ(seq2.size(), shortObs_.size());
    // Log probabilities should be negative (since they're log of probabilities < 1)
    EXPECT_LT(logProb1, 0.0);
    EXPECT_LT(logProb2, 0.0);
}
...
TEST_F(CalculatorTraitsTest, AutoCalculator) {
    ...
    // Test basic Viterbi functionality
    EXPECT_NO_THROW(autoCalc.decode());
    EXPECT_NO_THROW(autoCalc.getLogProbability());
    EXPECT_NO_THROW(autoCalc.getStateSequence());
...
TEST_F(CalculatorTraitsTest, CalculatorCorrectness) {
    ...
    // All should produce reasonable Viterbi results
    const StateSequence standardSeq = standardAuto.decode();
    const StateSequence scaledSimdSeq = scaledSimdAuto.decode();
    const StateSequence logSimdSeq = logSimdAuto.decode();
    
    const double standardLogProb = standardAuto.getLogProbability();
    const double scaledSimdLogProb = scaledSimdAuto.getLogProbability();
    const double logSimdLogProb = logSimdAuto.getLogProbability();
    
    std::cout << "Viterbi results comparison:" << std::endl;
    std::cout << "Standard log prob: " << standardLogProb << std::endl;
    std::cout << "Scaled SIMD log prob: " << scaledSimdLogProb << std::endl;
    std::cout << "Log SIMD log prob: " << logSimdLogProb << std::endl;
    
    // All should produce valid sequences
    EXPECT_EQ(standardSeq.size(), shortObs_.size());
    EXPECT_EQ(scaledSimdSeq.size(), shortObs_.size());
    EXPECT_EQ(logSimdSeq.size(), shortObs_.size());
    
    // All log probabilities should be negative
    EXPECT_LT(standardLogProb, 0.0);
    EXPECT_LT(scaledSimdLogProb, 0.0);
    EXPECT_LT(logSimdLogProb, 0.0);
}
