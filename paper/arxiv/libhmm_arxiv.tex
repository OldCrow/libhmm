...
GHMM was benchmarked on a separate macOS system (macOS Catalina 10.15,
Intel Core i7-3820QM, AppleClang, AVX --- the highest SIMD tier on that
...
\midrule
Elk movement   & 14,394 obs., 2 states & 99 ms   & $\approx$2{,}000 ms & $\approx$20$\times$ \\
DAX regimes    & 5,838 obs., 3 states  & 2 s     & $\approx$1{,}360 s  & $\approx$680$\times$ \\
Earthquake     & 107 obs., 2 states    & 4 ms    & $\approx$20 ms      & $\approx$5$\times$ \\
...
\texttt{libhmm} timings were measured on a Windows system with a Ryzen~7
processor (AVX-512, MSVC Release build).
...
the DAX benchmark is therefore conservative (the reference hardware is
faster than Ivy Bridge generation).
