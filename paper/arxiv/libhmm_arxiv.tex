...
\texttt{libhmm} was originally developed during the first author's graduate
...
\texttt{libhmm} & C++ & AVX-512/AVX2/SSE2/NEON & 16 continuous+discrete & None & Yes & C++20 \\
\bottomrule
\end{tabular}
\end{table}
...
Catalina 10.15, Intel Core i7-3820QM, AppleClang, AVX --- the highest
SIMD tier on that machine), with \texttt{libhmm} built on the same
machine to obtain a same-hardware comparison.
On this platform, GHMM 0.9-rc3 achieves approximately $9.2\times$ the
forward-backward throughput of \texttt{libhmm} on the same hardware
(20,545 vs.\ 2,235~obs/ms, averaged over $T \in \{10^3, \ldots, 10^6\}$,
Dishonest Casino and Weather problems).
The larger ratio relative to HMMLib on the primary platform (9.2$\times$
vs.\ 3.2$\times$) reflects the weaker SIMD tier: the macOS machine
has AVX only, not AVX2 or AVX-512, so \texttt{libhmm}'s log-space
compute is less amortised by vectorisation.
On the Ryzen~7 with AVX-512, the GHMM:libhmm ratio would be smaller.
...
Dagger (\dag) denotes timings from macOS Catalina (Intel i7-3820QM, AppleClang,
AVX) pending re-verification on the primary Windows platform.}
\label{tab:timing}
\begin{tabular}{llrrl}
\toprule
Benchmark & Dataset & \texttt{libhmm} & R package & Speedup \\
\midrule
Elk movement$^\dag$  & 14,394 obs., 2 states & 99 ms   & $\approx$2{,}000 ms & $\approx$20$\times$ \\
DAX regimes          & 5,838 obs., 3 states  & 2 s     & $\approx$1{,}360 s  & $\approx$680$\times$ \\
S\&P 500             & 5,786 obs., 3 states  & 0.7 s   & ---                 & ---                 \\
Earthquake$^\dag$    & 107 obs., 2 states    & 4 ms    & $\approx$20 ms      & $\approx$5$\times$ \\
...
DAX, S\&P~500, and wind direction timings were measured on Windows
(AMD Ryzen~7 7745, AVX-512, MSVC Release build).
Elk and earthquake timings are from macOS Catalina (Intel Core i7-3820QM,
AppleClang, AVX) and are marked with $\dag$ pending re-run on the primary
platform; parameter recovery results are hardware-independent.
The \texttt{fHMM} reference was measured on an Intel Ivy Bridge machine
reported in \citet{Oelschlager2024}; the $\approx$680$\times$ speedup for
the DAX benchmark is therefore conservative.

\subsubsection{Animal movement ecology: elk GPS tracks}
\label{sec:elk}

\citet{Morales2004} fit a 2-state HMM to four elk GPS tracks
(14,394 step-length and turning-angle observations) using
Gamma (step) and von Mises (angle) joint emissions.
This is the canonical benchmark dataset for \texttt{moveHMM}
\citep{Michelot2016}.
...
Encamped step SD (m)     & 401  & 403  & $<$1\% \\
Travelling step mean (m) & 1741 & 1751 & $<$1\% \\
Travelling step SD (m)   & 1519 & 1527 & $<$1\% \\
Encamped $\kappa$        & 0.595 & 0.592 & 0.5\% \\
Travelling $\kappa$      & 0.782 & 0.780 & 0.3\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{../figures/figure3_wind_boundary.pdf}
