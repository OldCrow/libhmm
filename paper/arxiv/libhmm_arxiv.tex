...
Hardware: Windows Ryzen~9, AVX-512, MSVC Release build.
...
Direct comparison with GHMM was not performed in this environment:
GHMM requires autotools, libxml2, and GSL, which are not available in
the Windows build context used here.
Based on its C implementation with flat array layout and no virtual
dispatch, GHMM is expected to match or exceed HMMLib on discrete
forward-backward throughput.
For \texttt{libhmm}'s target applications --- continuous emission
distributions where the emission evaluation cost dominates the FB
inner loop --- this gap is not relevant to practical performance.
