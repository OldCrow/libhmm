...
int main() {
    ...
    for (int size : problemSizes) {
        ...
        for (int iter = 0; iter < 1000; ++iter) {
            performance::ParallelUtils::parallelFor(
                0, size, [&](std::size_t i) { result[i] = data[i] * 2.0 + 1.0; }, 1);
        }
        ...
        std::cout << size << "\t" << std::fixed << std::setprecision(1) << seqTime << "\t\t"
                  << parTime << "\t\t" << overhead << "\t\t" << speedup << "x\n";
    }
