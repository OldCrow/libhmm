...
double NumericalSafety::clampLogProbability(double logProb) noexcept {
    ...
    return std::clamp(logProb, NumericalConstants::MIN_LOG_PROBABILITY,
                      NumericalConstants::MAX_LOG_PROBABILITY);
}
    ...
    for (std::size_t i = 0; i < matrix.size1(); ++i) {
        for (std::size_t j = 0; j < matrix.size2(); ++j) {
            if (!std::isfinite(matrix(i, j))) {
                throw std::runtime_error("Non-finite value detected in " + name + " at position (" +
...
void NumericalSafety::checkVectorFinite(const Vector &vector, const std::string &name) {
    for (std::size_t i = 0; i < vector.size(); ++i) {
        if (!std::isfinite(vector(i))) {
            throw std::runtime_error("Non-finite value detected in " + name + " at position " +
                                     std::to_string(i) + ": " + std::to_string(vector(i)));
        }
    }
}
...
ConvergenceDetector::ConvergenceDetector(double tolerance, std::size_t maxIterations,
                                         std::size_t windowSize)
    : tolerance_(tolerance), maxIterations_(maxIterations), windowSize_(windowSize),
      currentIteration_(0) {
...
AdaptivePrecision::AdaptivePrecision(double baseTolerance, std::size_t problemSize,
                                     bool useAdaptive)
    : baseTolerance_(baseTolerance), currentTolerance_(baseTolerance), problemSize_(problemSize),
      useAdaptive_(useAdaptive) {}
...
void AdaptivePrecision::updateTolerance(std::size_t iteration, double convergenceRate) {
    ...
    currentTolerance_ = std::clamp(currentTolerance_, baseTolerance_ * 0.1, baseTolerance_ * 100.0);
}
...
void AdaptivePrecision::scaleForData(double dataRange, std::size_t sequenceLength) {
    ...
    // Keep within reasonable bounds
    currentTolerance_ =
        std::clamp(currentTolerance_, baseTolerance_ * 0.01, baseTolerance_ * 1000.0);
}
        ...
        if (std::isnan(values(i))) {
            ...
            switch (strategy) {
                ...
                case RecoveryStrategy::ROBUST:
                    ...
                    if (i > 0 && i < values.size() - 1 && std::isfinite(values(i - 1)) &&
                        std::isfinite(values(i + 1))) {
