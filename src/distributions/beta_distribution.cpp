...
double BetaDistribution::incompleteBeta(double x, double a, double b) const noexcept {
    ...
    const double logBetaAB =
        std::lgamma(result_a) + std::lgamma(result_b) - std::lgamma(result_a + result_b);

    // Calculate prefix: x^a * (1-x)^b / (a * B(a,b))
    const double logPrefix = result_a * std::log(result_x) + result_b * std::log(1.0 - result_x) -
                             std::log(result_a) - logBetaAB;
    ...
    for (int m = 1; m <= maxIter; ++m) {
        // Even step (2m)
        double numerator =
            m * (result_b - m) * result_x / ((result_a + 2.0 * m - 1.0) * (result_a + 2.0 * m));
        d = 1.0 + numerator * d;
        if (std::abs(d) < 1e-30)
            d = 1e-30;
        c = 1.0 + numerator / c;
        ...
        // Odd step (2m+1)
        numerator = -(result_a + m) * (result_a + result_b + m) * result_x /
                    ((result_a + 2.0 * m) * (result_a + 2.0 * m + 1.0));
        d = 1.0 + numerator * d;
        if (std::abs(d) < 1e-30)
            d = 1e-30;
        c = 1.0 + numerator / c;
