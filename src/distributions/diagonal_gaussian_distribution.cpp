...
DiagonalGaussianDistribution::DiagonalGaussianDistribution(std::size_t dim, double mean, double var)
    ...
    if (var <= 0.0) {
        throw std::invalid_argument("DiagonalGaussianDistribution: initial variance must be > 0");
    }
    log_var_.resize(dim);
    inv_var_.resize(dim);
    updateCache();
...
double
DiagonalGaussianDistribution::getLogProbability(const ObservationVectorView &x) const noexcept {
    if (x.size() != dim_)
...
double DiagonalGaussianDistribution::getProbability(const ObservationVectorView &x) const {
...
double DiagonalGaussianDistribution::sample(std::mt19937_64 &) const {
    throw std::logic_error("DiagonalGaussianDistribution::sample() returns double which is not "
                           "meaningful for D-dimensional distributions. Use sample_mv().");
}

std::vector<double> DiagonalGaussianDistribution::sample_mv(std::mt19937_64 &rng) const {
    if (!isCacheValid())
        updateCache();
