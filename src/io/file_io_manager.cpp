...
void FileIOManager::copyFile(const std::filesystem::path &source,
    ...
    try {
        ...
        if (ec) {
            throw std::runtime_error("Failed to copy file from " + source.string() + " to " +
...
#endif
    ss << filepath.stem().string() << "_backup_" << std::put_time(&tm_buf, "%Y%m%d_%H%M%S")
...
bool FileIOManager::validatePath(const std::filesystem::path &filepath, bool checkRead,
...
FileIOManager::getFileSize(const std::filesystem::path &filepath) noexcept {
    try {
        if (!std::filesystem::exists(filepath)) {
            return std::nullopt;
        }
...
FileIOManager::getModificationTime(const std::filesystem::path &filepath) noexcept {
    try {
        if (!std::filesystem::exists(filepath)) {
            return std::nullopt;
        }
