...
public:
     ...
     */
    void batchAdd(Observation o) { observations.push_back(o); }

    /*
     * Recomputes the Centroid.
     */
    void recalculateCentroid() { centroid.setValue(observations); }
     ...
     */
    double getDistance(Observation o) const { return centroid.distance(o); }

    /*
     * Returns the number of Observations associated with the Cluster.
     */
    int size() { return static_cast<int>(observations.size()); }

    /*
     * Returns the centroid's value.
     */
    double getCentroidValue() { return centroid.getValue(); }

    void init(Observation o) { centroid.setValue(o); }
}; // class Cluster
