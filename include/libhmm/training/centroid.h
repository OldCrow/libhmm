...
public:
     ...
     */
    Centroid(Observation o) { value = o; }
     ...
     *
     */
    void add(Observation o, int size) { value = ((value * size) + o) / (size + 1); }
     ...
     *
     */
    void remove(Observation o, int size) { value = ((value * size) - o) / (size - 1); }

    /*
     * Returns the Euclidean (aka linear) distance from the centroid to the
     * Observation
	 */
    double distance(Observation o) const { return fabs(o - value); }
