...
private:
    ...
    [[nodiscard]] FbRecurrenceMode
    resolveRecurrenceMode(std::size_t numStates, std::size_t seqLen) const noexcept;
    ...
    static void init_log_forward(double* alphaData, const Vector& pi,
     ...
     */
    template<typename InnerFn>
    static void compute_backward_impl(std::size_t N, std::size_t T,
                                       const double* logTrans, const double* emitByTime,
...
template<typename Obs>
BasicForwardBackwardCalculator<Obs>::BasicForwardBackwardCalculator(
        const HmmType& hmm, const SeqType& observations)
    : Base(hmm, observations),
...
template<typename Obs>
BasicForwardBackwardCalculator<Obs>::BasicForwardBackwardCalculator(
        HmmType* hmm, const SeqType& observations)
    ...
    if constexpr (std::is_same_v<Obs, double>) {
        ...
        for (std::size_t i = 0; i < numStates_; ++i) {
            for (std::size_t t = 0; t < T; ++t) {
                logEmitBuf_[i * T + t] =
                    hmm.getDistribution(i).getLogProbability(row_view(obs, t));
            }
        }
    }
...
template<typename Obs>
FbRecurrenceMode BasicForwardBackwardCalculator<Obs>::resolveRecurrenceMode(
        std::size_t numStates, std::size_t seqLen) const noexcept
{
#if defined(LIBHMM_EXPERIMENT_FB_MAX_REDUCE)
    (void)numStates; (void)seqLen;
...
#else
    if (modeOverride_.has_value()) return *modeOverride_;
...
template<typename Obs>
void BasicForwardBackwardCalculator<Obs>::init_log_forward(
        double* alphaData, const Vector& pi, const double* emitRow0, std::size_t N) noexcept
{
...
template<typename Obs>
void BasicForwardBackwardCalculator<Obs>::init_log_backward(
        double* betaData, std::size_t T, std::size_t N) noexcept
{
    double* finalRow = betaData + (T - 1) * N;
    for (std::size_t i = 0; i < N; ++i) finalRow[i] = 0.0;
}

template<typename Obs>
template<typename InnerFn>
void BasicForwardBackwardCalculator<Obs>::compute_forward_impl(
        const HmmType& hmm, std::size_t N, std::size_t T,
        const double* logTransT, const double* emitByTime,
        Matrix& logAlpha, InnerFn inner)
{
...
template<typename Obs>
template<typename InnerFn>
void BasicForwardBackwardCalculator<Obs>::compute_backward_impl(
...
void BasicForwardBackwardCalculator<Obs>::computeLogForwardPairwise(std::size_t T) {
    compute_forward_impl(
        this->getHmmRef(), numStates_, T,
        logTransT_.data(), logEmitByTime_.data(), logAlpha_,
        [](const double* prev, const double* transCol, std::size_t n) noexcept {
            double s = LOG_ZERO;
            for (std::size_t i = 0; i < n; ++i) s = logSumExp(s, prev[i] + transCol[i]);
            return s;
        });
}
...
void BasicForwardBackwardCalculator<Obs>::computeLogForwardMaxReduce(std::size_t T) {
    ...
    compute_forward_impl(
        ...
        [](const double* prev, const double* transCol, std::size_t n) noexcept {
            ...
            if (!std::isfinite(m)) return LOG_ZERO;
...
void BasicForwardBackwardCalculator<Obs>::computeLogBackwardPairwise(std::size_t T) {
    compute_backward_impl(
        numStates_, T, logTrans_.data(), logEmitByTime_.data(), logBeta_,
        [](const double* transRow,
           const double* emitNext,
           const double* nextBeta,
