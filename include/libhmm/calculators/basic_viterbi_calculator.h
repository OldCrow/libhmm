...
template<typename Obs>
BasicViterbiCalculator<Obs>::BasicViterbiCalculator(
...
void BasicViterbiCalculator<Obs>::runViterbi(std::size_t T) {
    ...
    for (std::size_t t = 1; t < T; ++t) {
        ...
        for (std::size_t j = 0; j < N; ++j) {
            ...
            for (std::size_t i = 0; i < N; ++i) {
                ...
                if (val > bestVal) { bestVal = val; bestFrom = static_cast<int>(i); }
            }
    ...
    for (std::size_t i = 0; i < N; ++i) {
        if (finalRow[i] > bestVal) { bestVal = finalRow[i]; bestLast = static_cast<int>(i); }
    }
