...
public:
    ...
    static bool isProbabilityDistribution(
        const Vector &probs,
...
public:
    ...
    explicit ConvergenceDetector(
        double tolerance = NumericalConstants::DEFAULT_CONVERGENCE_TOLERANCE,
...
public:
    ...
    explicit AdaptivePrecision(
        double baseTolerance = NumericalConstants::DEFAULT_CONVERGENCE_TOLERANCE,
        std::size_t problemSize = 1, bool useAdaptive = true);
...
public:
    ...
    static bool recoverFromUnderflow(Vector &probs,
                                     RecoveryStrategy strategy = RecoveryStrategy::GRACEFUL);

    /// Attempt to recover from overflow in exponential calculations
    /// @param values Values that may have caused overflow
    /// @param strategy Recovery strategy to use
    /// @return True if recovery was successful
    static bool recoverFromOverflow(Vector &values,
                                    RecoveryStrategy strategy = RecoveryStrategy::GRACEFUL);
    ...
    static std::size_t handleNaNValues(Vector &values,
                                       RecoveryStrategy strategy = RecoveryStrategy::GRACEFUL);
    ...
    static bool repairDegenerateMatrix(Matrix &matrix,
                                       RecoveryStrategy strategy = RecoveryStrategy::GRACEFUL);
