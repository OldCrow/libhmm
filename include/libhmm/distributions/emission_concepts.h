...
concept EmissionDistributionFor =
    requires(D &d, const D &cd, typename BasicEmissionDistribution<Obs>::obs_param_t x,
             std::span<const Obs> data, std::span<const double> weights, std::span<double> out) {
        ...
        { cd.clone() } -> std::same_as<std::unique_ptr<BasicEmissionDistribution<Obs>>>;
    };
