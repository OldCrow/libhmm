...
template <typename T>
struct supports_fitting<
    T, std::void_t<decltype(std::declval<T>().fit(std::declval<std::vector<double> &>()))>>
    : std::true_type {};
...
template <typename T>
struct has_parameter_estimation<T, std::void_t<decltype(std::declval<T>().getMean()),
                                               decltype(std::declval<T>().getVariance())>>
    : std::true_type {};

/// Type trait to check if a distribution is suitable for Baum-Welch training
template <typename T>
struct is_baum_welch_compatible
    : std::bool_constant<is_discrete_distribution<T>::value && supports_fitting<T>::value> {};

/// Type trait to check if a distribution is suitable for Viterbi training
template <typename T>
struct is_viterbi_compatible : std::bool_constant<(is_discrete_distribution<T>::value ||
                                                   is_continuous_distribution<T>::value) &&
...
enum class DistributionCategory { Discrete, Continuous, Unknown };
...
struct distribution_validator {
    ...
    static_assert(supports_fitting_v<T>, "Distribution must support fitting operation");
};
} // namespace detail

/// Macro for compile-time distribution validation
#define LIBHMM_VALIDATE_DISTRIBUTION(T)                                                            \
    static_assert(libhmm::is_discrete_distribution_v<T> ||                                         \
                      libhmm::is_continuous_distribution_v<T>,                                     \
                  #T " must be a recognized distribution type");                                   \
    static_assert(libhmm::supports_fitting_v<T>, #T " must support fitting operation")
...
struct trainer_selector {
    ...
    using type = std::conditional_t<is_discrete_distribution_v<DistType>,
