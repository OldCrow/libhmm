...
private:
    ...
    void createLAMPConfigFile(ProblemType &problem, const string &config_file,
                              const string &abs_sequence_file, const string &abs_output_prefix) {
        ...
        while (getline(file, line)) {
            ...
            if (line.find("-Log Prob of all") != string::npos) {
                ...
                if (pos != string::npos) {
                    try {
                        likelihood =
                            -stod(line.substr(pos + 1)); // Convert to positive log likelihood
                    } catch (...) {
...
void printPerformanceComparison(const vector<BenchmarkResults> &results) {
    ...
    cout << left << setw(25) << "Problem" << setw(10) << "Length" << setw(16) << "libhmm (ms)"
         << setw(16) << "LAMP (ms)" << setw(12) << "Speedup" << endl;
    cout << string(80, '-') << endl;
        ...
        if (group.second.size() >= 2) {
            ...
            if (libhmm_time > 0 && lamp_time > 0) {
                ...
                cout << left << setw(25) << problem_name << setw(10) << length << setw(16) << fixed
                     << setprecision(3) << libhmm_time << setw(16) << lamp_time << setprecision(2)
                     << speedup << "x" << endl;
            }
        }
    }
...
int main() {
    ...
    if (libhmm_successes > 0 && lamp_successes > 0) {
        double avg_speedup =
            (lamp_total_throughput / lamp_successes) / (libhmm_total_throughput / libhmm_successes);
        cout << "Overall performance ratio: " << setprecision(2) << avg_speedup << "x (LAMP/libhmm)"
             << endl;
    }
