...
public:
    ...
    struct SpeechProblem {
        ...
        vector<vector<double>> transition_matrix = {
            {0.6, 0.2, 0.1, 0.1}, {0.1, 0.6, 0.2, 0.1}, {0.1, 0.1, 0.6, 0.2}, {0.2, 0.1, 0.1, 0.6}};
        vector<vector<double>> emission_matrix = {
            // 8 acoustic features
...
public:
    ...
    BenchmarkResults runProblemComparison(ProblemType &problem, int sequence_length) {
        ...
        results.likelihood_difference =
            abs(results.libhmm_viterbi_likelihood - results.hmmlib_viterbi_likelihood);
        results.numerical_match = results.likelihood_difference < TOLERANCE;

        cout << "Viterbi Likelihood difference: " << scientific << results.likelihood_difference
             << endl;
...
private:
    template <typename ProblemType>
    void runLibHmmBenchmark(const ProblemType &problem, const vector<unsigned int> &obs_sequence,
                            BenchmarkResults &results) {
        try {
            // Create libhmm HMM
            ...
            double forward_backward_likelihood = fb_calc.getLogProbability();

            auto end = high_resolution_clock::now();
            results.libhmm_forward_time = duration_cast<microseconds>(end - start).count() / 1000.0;
            results.libhmm_forward_likelihood =
                forward_backward_likelihood; // Store Forward-Backward result
            ...
            cout << "  libhmm - Forward: " << fixed << setprecision(3)
                 << results.libhmm_forward_time << "ms, Viterbi: " << results.libhmm_viterbi_time
                 << "ms" << endl;
            cout << "  libhmm - Forward-Backward Log-likelihood: " << scientific
                 << results.libhmm_forward_likelihood << endl;
            cout << "  libhmm - Viterbi Log-likelihood: " << scientific
                 << results.libhmm_viterbi_likelihood << endl;
    ...
    template <typename ProblemType>
    void runHmmLibBenchmark(const ProblemType &problem, const vector<unsigned int> &obs_sequence,
                            BenchmarkResults &results) {
        try {
            ...
            results.hmmlib_forward_likelihood =
                forward_backward_likelihood; // Store Forward-Backward result
            ...
            cout << "  HMMLib - Forward: " << fixed << setprecision(3)
                 << results.hmmlib_forward_time << "ms, Viterbi: " << results.hmmlib_viterbi_time
                 << "ms" << endl;
            cout << "  HMMLib - Forward-Backward Log-likelihood: " << scientific
                 << results.hmmlib_forward_likelihood << endl;
            cout << "  HMMLib - Viterbi Log-likelihood: " << scientific
                 << results.hmmlib_viterbi_likelihood << endl;
...
void printResultsTable(const vector<BenchmarkResults> &results) {
    ...
    cout << left << setw(22) << "Problem" << right << setw(8) << "Length" << right << setw(13)
         << "libhmm F-B" << right << setw(13) << "HMMLib F-B" << right << setw(14)
         << "HMMLib/libhmm" << right << setw(13) << "libhmm Vit" << right << setw(13)
         << "HMMLib Vit" << right << setw(14) << "HMMLib/libhmm" << right << setw(10) << "Match"
         << endl;
    cout << left << setw(22) << "" << right << setw(8) << "" << right << setw(13) << "(ms)" << right
         << setw(13) << "(ms)" << right << setw(14) << "F-B Ratio" << right << setw(13) << "(ms)"
         << right << setw(13) << "(ms)" << right << setw(14) << "Vit Ratio" << right << setw(10)
         << "Numeric" << endl;
    cout << string(130, '-') << endl;
    ...
    for (const auto &result : results) {
        ...
        cout << left << setw(22) << result.problem_name << right << setw(8)
             << result.sequence_length << right << setw(13) << fixed << setprecision(2)
             << result.libhmm_forward_time << right << setw(13) << setprecision(2)
             << result.hmmlib_forward_time << right << setw(14) << fb_speedup_str.str() << right
             << setw(13) << setprecision(2) << result.libhmm_viterbi_time << right << setw(13)
             << setprecision(2) << result.hmmlib_viterbi_time << right << setw(14)
             << vit_speedup_str.str() << right << setw(10)
             << (result.numerical_match ? "YES" : "NO") << endl;
    }
...
void printLikelihoodComparison(const vector<BenchmarkResults> &results) {
    ...
    cout << string(90, '-') << endl;
    cout << left << setw(25) << "Problem" << setw(8) << "Length" << setw(20) << "libhmm Likelihood"
         << setw(20) << "HMMLib Likelihood" << setw(15) << "Difference" << endl;
    cout << string(90, '-') << endl;
    ...
    for (const auto &result : results) {
        ...
        cout << left << setw(25) << result.problem_name << setw(8) << result.sequence_length
             << setw(20) << scientific << setprecision(6) << result.libhmm_viterbi_likelihood
             << setw(20) << result.hmmlib_viterbi_likelihood << setw(15)
             << result.likelihood_difference << endl;
    }
    cout << string(90, '=') << endl;
}
...
int main() {
    ...
    if (successful_comparisons > 0) {
        cout << "\nSUMMARY" << endl;
        cout << "-------" << endl;
        cout << "Successful comparisons: " << successful_comparisons << "/" << results.size()
             << endl;
        cout << "Numerical matches: " << numerical_matches << "/" << successful_comparisons << " ("
             << fixed << setprecision(1) << (100.0 * numerical_matches / successful_comparisons)
             << "%)" << endl;
        cout << "Average Forward-Backward speedup: " << setprecision(2)
             << total_fb_speedup / successful_comparisons << "x" << endl;
        cout << "Average Viterbi speedup: " << total_vit_speedup / successful_comparisons << "x"
             << endl;
        ...
        if (avg_fb_speedup > 1.0) {
            cout << "\nOverall: HMMLib is " << setprecision(1) << avg_fb_speedup
                 << "x faster on average for Forward-Backward" << endl;
        } else {
            cout << "\nOverall: libhmm is " << setprecision(1) << (1.0 / avg_fb_speedup)
                 << "x faster on average for Forward-Backward" << endl;
        }

        if (avg_vit_speedup > 1.0) {
            cout << "Overall: HMMLib is " << setprecision(1) << avg_vit_speedup
                 << "x faster on average for Viterbi" << endl;
        } else {
            cout << "Overall: libhmm is " << setprecision(1) << (1.0 / avg_vit_speedup)
                 << "x faster on average for Viterbi" << endl;
        }
    }
