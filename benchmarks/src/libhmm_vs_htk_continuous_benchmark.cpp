    ...
    struct GaussianSpeechProblem {
        ...
        vector<vector<double>> generateSequence(int length) {
            ...
            for (int i = 0; i < length; ++i) {
                ...
                normal_distribution<double> obs_dist(means[current_state],
                                                     sqrt(variances[current_state]));
...
public:
    ...
    BenchmarkResults runBenchmark(ProblemType &problem,
                                  const vector<vector<double>> &full_obs_sequence,
                                  int sequence_length) {
...
private:
    ...
    double parseHTKLogLikelihood(const string &output_file) {
        ...
        while (getline(file, line)) {
            if (line.find("Log Probability") != string::npos || line.find("LogP") != string::npos ||
...
public:
    ...
    BenchmarkResults runBenchmark(ProblemType &problem,
        ...
        try {
            ...
            string model_file =
                temp_dir + "/" + problem.name + "_" + to_string(sequence_length) + ".hmm";
            string data_file =
                temp_dir + "/" + problem.name + "_" + to_string(sequence_length) + ".htk";
            string script_file =
                temp_dir + "/" + problem.name + "_" + to_string(sequence_length) + ".scp";
            string output_file =
                temp_dir + "/" + problem.name + "_" + to_string(sequence_length) + ".out";
            string viterbi_file =
                temp_dir + "/" + problem.name + "_" + to_string(sequence_length) + ".rec";
            ...
            string hvite_cmd = "HVite -A -T 1 -H " + model_file + " -S " + script_file + " -i " +
                               viterbi_file + " -w /dev/null continuous_model > " + output_file +
                               " 2>&1";
...
void printComparisonResults(const vector<BenchmarkResults> &results) {
    ...
    cout << left << setw(12) << "Library" << setw(25) << "Problem" << setw(10) << "Length"
         << setw(15) << "Forward-B (ms)" << setw(15) << "Viterbi (ms)" << setw(20)
         << "Throughput (obs/ms)" << setw(15) << "Success" << setw(20) << "Log-likelihood" << endl;
    cout << string(120, '-') << endl;
    ...
    for (const auto &result : results) {
        ...
        cout << left << setw(12) << result.library_name << setw(25) << result.problem_name
             << setw(10) << result.sequence_length << setw(15) << fixed << setprecision(3)
             << result.forward_time << setw(15) << result.viterbi_time << setw(20)
             << setprecision(1) << throughput << setw(15) << (result.success ? "YES" : "NO")
...
void printPerformanceComparison(const vector<BenchmarkResults> &results) {
    ...
    cout << left << setw(25) << "Problem" << setw(10) << "Length" << setw(16) << "libhmm (ms)"
         << setw(16) << "HTK (ms)" << setw(12) << "Speedup" << endl;
    cout << string(80, '-') << endl;
    ...
    for (const auto &group : grouped_results) {
        if (group.second.size() >= 2) {
            ...
            if (libhmm_time > 0 && htk_time > 0) {
                ...
                cout << left << setw(25) << problem_name << setw(10) << length << setw(16) << fixed
                     << setprecision(3) << libhmm_time << setw(16) << htk_time << setprecision(2)
                     << speedup << "x" << endl;
            }
        }
    }
...
int main() {
    ...
    for (int length : test_lengths) {
        ...
        auto libhmm_result =
            libhmm_benchmark.runBenchmark(speech, speech_full_obs_sequence, length);
    ...
    for (int length : test_lengths) {
        cout << "  libhmm (length: " << length << "): ";
        auto libhmm_result =
            libhmm_benchmark.runBenchmark(temperature, temperature_full_obs_sequence, length);
        cout << (libhmm_result.success ? "SUCCESS" : "FAILED") << endl;
        results.push_back(libhmm_result);

        cout << "  HTK (length: " << length << "): ";
        auto htk_result =
            htk_benchmark.runBenchmark(temperature, temperature_full_obs_sequence, length);
        cout << (htk_result.success ? "SUCCESS" : "FAILED") << endl;
        results.push_back(htk_result);
    }
    ...
    if (libhmm_successes > 0 && htk_successes > 0) {
        double avg_speedup =
            (htk_total_throughput / htk_successes) / (libhmm_total_throughput / libhmm_successes);
        cout << "Overall performance ratio: " << setprecision(2) << avg_speedup << "x (HTK/libhmm)"
             << endl;
    }
