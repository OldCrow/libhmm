...
public:
    ...
    struct GaussianSignalModel {
        ...
        vector<vector<double>> transition_matrix = {{0.7, 0.3}, {0.4, 0.6}};
    ...
    GaussianSignalModel getModelParameters() { return GaussianSignalModel(); }
};
...
BenchmarkResult runLibhmmGaussian(const ContinuousGaussianProblems::GaussianSignalModel &model,
    ...
    try {
        ...
        cout << "  libhmm - Log-likelihood: " << scientific << setprecision(3)
...
BenchmarkResult runStochHMMGaussian(const ContinuousGaussianProblems::GaussianSignalModel &model,
    ...
    try {
        ...
        vector<double> params1 = {model.means[0],
                                  sqrt(model.variances[0])}; // State 1: mean, std_dev
        vector<double> params2 = {model.means[1],
                                  sqrt(model.variances[1])}; // State 2: mean, std_dev
        ...
        for (int t = 1; t < T; ++t) {
            ...
            double log_trans_sum1 =
                logSumExp(prev_log_alpha_0 + log_trans[0][0], prev_log_alpha_1 + log_trans[1][0]);
            log_alpha[t][0] = log_trans_sum1 + log_emission1;

            // State 1: log(alpha[t-1][0] * trans[0][1] + alpha[t-1][1] * trans[1][1])
            double log_trans_sum2 =
                logSumExp(prev_log_alpha_0 + log_trans[0][1], prev_log_alpha_1 + log_trans[1][1]);
        ...
        cout << "  StochHMM - Log-likelihood: " << scientific << setprecision(3)
             << result.log_likelihood << endl;
...
void printResults(const vector<BenchmarkResult> &results) {
    ...
    cout << "======================================================================================"
            "==================================\n";
    cout << left << setw(10) << "Library" << setw(25) << "Problem" << setw(8) << "Length"
         << setw(15) << "Forward-B (ms)" << setw(15) << "Viterbi (ms)" << setw(20)
         << "Throughput (obs/ms)" << setw(10) << "Success" << setw(20) << "Log-likelihood" << endl;
    cout << "--------------------------------------------------------------------------------------"
            "----------------------------------\n";

    for (const auto &result : results) {
        cout << left << setw(10) << result.library << setw(25) << result.problem << setw(8)
             << result.sequence_length << setw(15) << fixed << setprecision(3)
             << result.forward_time_ms << setw(15) << fixed << setprecision(3)
             << result.viterbi_time_ms << setw(20) << fixed << setprecision(1)
             << result.throughput_obs_per_ms << setw(10) << (result.success ? "YES" : "NO")
...
void analyzeNumericalAccuracy(const vector<BenchmarkResult> &results) {
    ...
    cout << "--------------------------------------------------------------------------------\n";
    cout << left << setw(25) << "Problem" << setw(10) << "Length" << setw(18) << "libhmm LogLik"
         << setw(18) << "StochHMM LogLik" << setw(18) << "Difference" << endl;
    cout << "--------------------------------------------------------------------------------\n";
    ...
    for (size_t i = 0; i < results.size(); i += 2) {
        if (i + 1 < results.size() && results[i].success && results[i + 1].success) {
            ...
            cout << left << setw(25) << results[i].problem << setw(10) << results[i].sequence_length
...
int main() {
    ...
    for (int length : test_lengths) {
        ...
        if (libhmm_result.success && stochhmm_result.success) {
            double likelihood_diff =
                abs(libhmm_result.log_likelihood - stochhmm_result.log_likelihood);
            cout << "Log-likelihood difference: " << scientific << setprecision(3)
                 << likelihood_diff << "\n";
    ...
    cout << "--------------------------------------------------------------------------------\n";
    cout << left << setw(25) << "Problem" << setw(10) << "Length" << setw(15) << "libhmm (ms)"
         << setw(15) << "StochHMM (ms)" << setw(15) << "Speedup" << endl;
    cout << "--------------------------------------------------------------------------------\n";
    ...
    for (size_t i = 0; i < results.size(); i += 2) {
        if (i + 1 < results.size() && results[i].success && results[i + 1].success) {
            ...
            cout << left << setw(25) << results[i].problem << setw(10) << results[i].sequence_length
                 << setw(15) << fixed << setprecision(3) << results[i].forward_time_ms << setw(15)
                 << fixed << setprecision(3) << results[i + 1].forward_time_ms << setw(15) << fixed
