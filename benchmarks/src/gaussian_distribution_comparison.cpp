...
void printComparisonResults(const vector<DistributionResult> &results,
                            const GaussianTestCase &test_case) {
    cout << "\n===================================================================================="
            "\n";
...
void analyzeNumericalDifferences(const vector<DistributionResult> &results,
                                 const GaussianTestCase &test_case) {
    ...
    for (size_t i = 0; i < test_case.test_values.size(); ++i) {
        ...
        if (i < successful_results[0].pdf_values.size() &&
            ...
            double pdf_diff =
                abs(successful_results[0].pdf_values[i] - successful_results[1].pdf_values[i]);
            double log_pdf_diff = abs(successful_results[0].log_pdf_values[i] -
                                      successful_results[1].log_pdf_values[i]);
...
int main() {
    ...
    vector<GaussianTestCase> test_cases = {{0.0,                         // mean
