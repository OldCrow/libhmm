...
public:
    ...
    BenchmarkResults runBenchmark(ProblemType &problem,
        ...
        try {
            ...
            string data_file =
                temp_dir + "/" + problem.name + "_" + to_string(sequence_length) + ".dat";
            string script_file =
            ...
            string hvite_cmd = "HVite -A -T 1 -H " + model_file + " -S " + script_file + " -i " +
                               viterbi_file + " -w /dev/null casino > " + output_file + " 2>&1";
...
int main() {
    ...
    for (int length : test_lengths) {
        ...
        auto libhmm_result =
            libhmm_benchmark.runBenchmark(casino, casino_full_obs_sequence, length);
    ...
    for (int length : test_lengths) {
        cout << "  libhmm (length: " << length << "): ";
        auto libhmm_result =
            libhmm_benchmark.runBenchmark(weather, weather_full_obs_sequence, length);
        cout << (libhmm_result.success ? "SUCCESS" : "FAILED") << endl;
        results.push_back(libhmm_result);
