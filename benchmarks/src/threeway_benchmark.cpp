...
void printComparisonResults(const vector<BenchmarkResults> &results) {
    ...
    cout << left << setw(12) << "Library" << setw(25) << "Problem" << setw(10) << "Length"
         ...
         << "Throughput (obs/ms)" << setw(15) << "Success" << setw(25) << "Log-likelihood" << endl;
    cout << string(140, '-') << endl;
...
void printNumericalAccuracyAnalysis(const vector<BenchmarkResults> &results) {
    ...
    cout << left << setw(25) << "Problem" << setw(10) << "Length" << setw(18) << "libhmm LogLik"
         << setw(18) << "HMMLib LogLik" << setw(18) << "StochHMM LogLik" << endl;
    cout << string(80, '-') << endl;
    ...
    for (const auto &group : grouped_results) {
        if (group.second.size() >= 3) {
            ...
            cout << left << setw(25) << problem_name << setw(10) << length << setw(18) << scientific
                 << setprecision(6) << libhmm_loglik << setw(18) << hmmlib_loglik << setw(18)
                 << stochhmm_loglik << endl;
...
int main() {
    ...
    for (int length : test_lengths) {
        ...
        cout << "    HMMLib: ";
        auto hmmlib_result =
            hmmlib_benchmark.runBenchmark(casino, casino_full_obs_sequence, length);
        cout << (hmmlib_result.success ? "SUCCESS" : "FAILED") << endl;
        results.push_back(hmmlib_result);

        cout << "    StochHMM: ";
        auto stochhmm_result =
            stochhmm_benchmark.runBenchmark(casino, casino_full_obs_sequence, length);
    ...
    for (int length : test_lengths) {
        ...
        cout << "    HMMLib: ";
        auto hmmlib_result =
            hmmlib_benchmark.runBenchmark(weather, weather_full_obs_sequence, length);
        cout << (hmmlib_result.success ? "SUCCESS" : "FAILED") << endl;
        results.push_back(hmmlib_result);

        cout << "    StochHMM: ";
        auto stochhmm_result =
            stochhmm_benchmark.runBenchmark(weather, weather_full_obs_sequence, length);
        cout << (stochhmm_result.success ? "SUCCESS" : "FAILED") << endl;
        results.push_back(stochhmm_result);
    }
