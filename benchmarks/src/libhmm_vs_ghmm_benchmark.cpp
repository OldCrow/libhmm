...
public:
    ...
    BenchmarkResults runBenchmark(ProblemType &problem,
        ...
        try {
            ...
            start = high_resolution_clock::now();
            int *viterbi_path = ghmm_dmodel_viterbi(&model, ghmm_sequence, obs_sequence.size(),
                                                    &pathlen, &viterbi_log_prob);
            end = high_resolution_clock::now();
            results.viterbi_time = duration_cast<microseconds>(end - start).count() / 1000.0;
...
void printPerformanceComparison(const vector<BenchmarkResults> &results) {
    ...
    cout << left << setw(25) << "Problem" << setw(10) << "Length" << setw(16) << "libhmm (ms)"
         << setw(16) << "GHMM (ms)" << setw(12) << "Speedup" << endl;
    cout << string(80, '-') << endl;
        ...
        if (group.second.size() >= 2) {
            ...
            if (libhmm_time > 0 && ghmm_time > 0) {
                ...
                cout << left << setw(25) << problem_name << setw(10) << length << setw(16) << fixed
                     << setprecision(3) << libhmm_time << setw(16) << ghmm_time << setprecision(2)
                     << speedup << "x" << endl;
            }
        }
    }
...
int main() {
    ...
    if (libhmm_successes > 0 && ghmm_successes > 0) {
        double avg_speedup =
            (ghmm_total_throughput / ghmm_successes) / (libhmm_total_throughput / libhmm_successes);
        cout << "Overall performance ratio: " << setprecision(2) << avg_speedup << "x (GHMM/libhmm)"
             << endl;
    }
