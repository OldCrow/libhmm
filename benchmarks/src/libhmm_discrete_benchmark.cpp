...
public:
    ...
    BenchmarkResults runBenchmark(ProblemType &problem, int sequence_length) {
        ...
        try {
            ...
            results.likelihood =
                forward_backward_log_likelihood; // Numerically stable log-likelihood
...
void printResultsTable(const vector<BenchmarkResults> &results) {
    ...
    cout << left << setw(25) << "Problem" << setw(8) << "Length" << setw(15) << "Forward-B (ms)"
         << setw(15) << "Viterbi (ms)" << setw(15) << "Throughput (obs/ms)" << setw(12) << "Success"
         << endl;
    cout << string(90, '-') << endl;
    ...
    for (const auto &result : results) {
        ...
        cout << left << setw(25) << result.problem_name << setw(8) << result.sequence_length
             << setw(15) << fixed << setprecision(3) << result.forward_time << setw(15)
             << result.viterbi_time << setw(15) << setprecision(1) << throughput << setw(12)
             << (result.success ? "YES" : "NO") << endl;
    }
...
void printScalingAnalysis(const vector<BenchmarkResults> &results) {
    ...
    cout << string(70, '-') << endl;
    cout << left << setw(25) << "Problem" << setw(8) << "Length" << setw(20) << "Log-likelihood"
    ...
    for (const auto &result : results) {
        ...
        cout << left << setw(25) << result.problem_name << setw(8) << result.sequence_length
             << setw(20) << scientific << setprecision(6) << result.likelihood << setw(15) << fixed
             << setprecision(2) << time_per_obs << endl;
    }
    cout << string(70, '=') << endl;
}
...
int main() {
    ...
    if (successful_tests > 0) {
        ...
        cout << "Best performance: " << setprecision(1) << best_throughput << " obs/ms ("
             << best_problem << ")" << endl;

        // Analyze scaling behavior
        cout << "\nSCALING BEHAVIOR:" << endl;
        for (const string &problem_name :
             {"Dishonest Casino", "Weather Model", "CpG Island Detection", "Speech Recognition"}) {
            ...
            if (problem_results.size() >= 2) {
                ...
                cout << "  " << problem_name << ": " << setprecision(2) << scaling_factor
                     << "x time increase from " << problem_results.front().first << " to "
                     << problem_results.back().first << " observations" << endl;
            }
        }
    }
