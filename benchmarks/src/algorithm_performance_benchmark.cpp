...
struct BenchmarkResult {
    ...
    void print() const {
        std::cout << std::setw(25) << test_name << std::setw(12) << library << std::setw(12)
                  << std::fixed << std::setprecision(3) << time_ms << "ms" << std::setw(15)
                  << std::scientific << std::setprecision(3) << likelihood << std::setw(8)
                  << num_states << std::setw(12) << seq_length << std::setw(12) << num_sequences
                  << std::endl;
    }
};
...
public:
    ...
    generateObservationSequences(size_t num_sequences, size_t seq_length, size_t alphabet_size) {
    ...
    BenchmarkResult benchmarkLibhmmForwardBackward(size_t num_states, size_t alphabet_size,
                                                   size_t seq_length) {
    ...
    BenchmarkResult benchmarkHMMLibForwardBackward(size_t num_states, size_t alphabet_size,
                                                   size_t seq_length) {
    ...
    BenchmarkResult benchmarkLibhmmViterbi(size_t num_states, size_t alphabet_size,
                                           size_t seq_length, bool use_simd = false) {
    ...
    BenchmarkResult benchmarkHMMLibViterbi(size_t num_states, size_t alphabet_size,
                                           size_t seq_length) {
    ...
    void runComprehensiveBenchmarks() {
        ...
        std::cout << std::setw(25) << "Test" << std::setw(12) << "Library" << std::setw(12)
                  << "Time" << std::setw(15) << "Likelihood" << std::setw(8) << "States"
                  << std::setw(12) << "Seq Length" << std::setw(12) << "Sequences" << std::endl;
        ...
        for (const auto &config : configs) {
            ...
            std::cout << "\n--- Configuration: " << num_states << " states, " << alphabet_size
                      << " symbols, " << seq_length << " length ---\n";

            // Forward-Backward comparison
            try {
                auto libhmm_fb =
                    benchmarkLibhmmForwardBackward(num_states, alphabet_size, seq_length);
                auto hmmlib_fb =
                    benchmarkHMMLibForwardBackward(num_states, alphabet_size, seq_length);
                ...
                std::cout << "    Forward-Backward Speedup: " << std::fixed << std::setprecision(2)
                          << speedup << "x "
                          << (speedup > 1 ? "(libhmm faster)" : "(HMMLib faster)") << std::endl;
            ...
            try {
                // Test scalar Viterbi
                auto libhmm_vit_scalar =
                    benchmarkLibhmmViterbi(num_states, alphabet_size, seq_length, false);
                ...
                std::cout << "    Viterbi Scalar Speedup: " << std::fixed << std::setprecision(2)
                          << speedup_scalar << "x "
                          << (speedup_scalar > 1 ? "(libhmm faster)" : "(HMMLib faster)")
                          << std::endl;

                // Test SIMD Viterbi if applicable
                if (num_states >= 4) {
                    auto libhmm_vit_simd =
                        benchmarkLibhmmViterbi(num_states, alphabet_size, seq_length, true);
                    ...
                    std::cout << "    Viterbi SIMD Speedup: " << std::fixed << std::setprecision(2)
                              << speedup_simd << "x "
                              << (speedup_simd > 1 ? "(libhmm faster)" : "(HMMLib faster)")
                              << std::endl;
                    ...
                    std::cout << "    SIMD vs Scalar: " << std::fixed << std::setprecision(2)
                              << simd_vs_scalar << "x "
                              << (simd_vs_scalar > 1 ? "(SIMD faster)" : "(Scalar faster)")
                              << std::endl;
                }
            } catch (const std::exception &e) {
    ...
    void saveResults(const std::string &filename) {
        ...
        for (const auto &result : results) {
            file << result.test_name << "," << result.library << "," << result.time_ms << ","
                 << result.likelihood << "," << result.num_states << "," << result.seq_length << ","
