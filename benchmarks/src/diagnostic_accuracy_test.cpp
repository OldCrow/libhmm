...
void testGaussianDistributionBasics() {
    ...
    cout << left << setw(10) << "x" << setw(18) << "libhmm PDF" << setw(18) << "Expected PDF"
         << setw(15) << "Difference" << setw(18) << "libhmm LogPDF" << setw(18) << "Expected LogPDF"
    ...
    for (const auto &test : test_cases) {
        ...
        cout << left << setw(10) << fixed << setprecision(1) << x << setw(18) << scientific
             << setprecision(9) << libhmm_pdf << setw(18) << scientific << setprecision(9)
             << expected_pdf << setw(15) << scientific << setprecision(3) << pdf_diff << setw(18)
             << scientific << setprecision(9) << libhmm_log_pdf << setw(18) << scientific
             << setprecision(9) << expected_log_pdf << setw(15) << scientific << setprecision(3)
             << log_pdf_diff << endl;
    }
...
void testGaussianParameterInterpretation() {
    ...
    cout << "  If constructor uses variance (4.0): PDF = " << scientific << setprecision(6)
         << pdf_variance_interp << endl;
    cout << "  If constructor uses std_dev (2.0):  PDF = " << scientific << setprecision(6)
         << pdf_stddev_interp << endl;

    // Calculate expected value manually for std_dev=2
    double expected_pdf = (1.0 / (2.0 * sqrt(2 * M_PI))) * exp(-0.5 * pow((3.0 - 1.0) / 2.0, 2));
    cout << "  Expected (manual std_dev=2):        PDF = " << scientific << setprecision(6)
         << expected_pdf << endl;
    ...
    cout << "  Difference (variance interpretation): " << scientific << setprecision(3)
         << diff_variance << endl;
    cout << "  Difference (std_dev interpretation):  " << scientific << setprecision(3)
         << diff_stddev << endl;
...
void testDiscreteDistributionLogProbability() {
    ...
    cout << left << setw(12) << "Observation" << setw(15) << "PDF" << setw(18) << "Log PDF"
         << setw(18) << "Expected Log" << setw(15) << "Difference" << endl;
    cout << string(78, '-') << endl;
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        cout << left << setw(12) << i << setw(15) << fixed << setprecision(6) << pdf << setw(18)
             << scientific << setprecision(6) << log_pdf << setw(18) << scientific
             << setprecision(6) << expected_log << setw(15) << scientific << setprecision(3) << diff
             << endl;
    }
    cout << endl;
}
...
void testHMMSetupConsistency() {
    ...
    for (int i = 0; i < 2; ++i) {
        for (int j = 0; j < 2; ++j) {
            cout << "  T[" << i << "," << j << "] = " << fixed << setprecision(6)
                 << retrieved_trans(i, j) << endl;
        }
    }
    ...
    for (int state = 0; state < 2; ++state) {
        for (int symbol = 0; symbol < 3; ++symbol) {
            ...
            cout << "  E[" << state << "," << symbol << "] = " << fixed << setprecision(6) << prob
                 << endl;
        }
    }
    cout << endl;
...
void testCalculatorConsistency() {
    ...
    try {
        ...
        cout << "ForwardBackward (ptr):    " << scientific << setprecision(10) << fb_log_ptr
             << endl;

        libhmm::ForwardBackwardCalculator fb_ref(*hmm, obs);
        double fb_log_ref = fb_ref.getLogProbability();
        cout << "ForwardBackward (ref):    " << scientific << setprecision(10) << fb_log_ref
             << endl;

        double fb_prob = fb_ptr.probability();
        double fb_from_prob = (fb_prob > 0.0) ? log(fb_prob) : -INFINITY;
        cout << "ForwardBackward (log p):  " << scientific << setprecision(10) << fb_from_prob
             << endl;
        ...
        cout << "Viterbi path log-prob:    " << scientific << setprecision(10) << viterbi_log
             << endl;
        ...
        cout << "  FB ptr vs ref:          " << scientific << setprecision(3) << ptr_ref_diff
             << endl;
        cout << "  FB log vs log(prob):    " << scientific << setprecision(3) << ptr_prob_diff
             << endl;
        cout << "  FB - Viterbi gap:       " << scientific << setprecision(3) << fb_viterbi_gap
             << endl;
...
void testManualProbabilityCalculation() {
    ...
    cout << "  A = [[" << A[0][0] << ", " << A[0][1] << "], [" << A[1][0] << ", " << A[1][1] << "]]"
         << endl;
    cout << "  B = [[" << B[0][0] << ", " << B[0][1] << ", " << B[0][2] << "], [" << B[1][0] << ", "
         << B[1][1] << ", " << B[1][2] << "]]" << endl;
    ...
    try {
        ...
        libhmm_trans(0, 0) = A[0][0];
        libhmm_trans(0, 1) = A[0][1];
        libhmm_trans(1, 0) = A[1][0];
        libhmm_trans(1, 1) = A[1][1];
        hmm->setTrans(libhmm_trans);

        auto dist0 = make_unique<libhmm::DiscreteDistribution>(3);
        dist0->setProbability(0, B[0][0]);
        dist0->setProbability(1, B[0][1]);
        dist0->setProbability(2, B[0][2]);
        auto dist1 = make_unique<libhmm::DiscreteDistribution>(3);
        dist1->setProbability(0, B[1][0]);
        dist1->setProbability(1, B[1][1]);
        dist1->setProbability(2, B[1][2]);
