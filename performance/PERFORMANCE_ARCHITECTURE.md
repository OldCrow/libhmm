...
All 16 scalar distribution TUs are listed in `LIBHMM_SIMD_SOURCES` (the 3 MV distributions are not — they call `getLogProbability` per observation rather than a batch interface). Both scalar and MV explicit-instantiation TUs for `BasicForwardBackwardCalculator` and `BasicBaumWelchTrainer` are also in `LIBHMM_SIMD_SOURCES`. in the top-level `CMakeLists.txt` and receive the SIMD compile flags.
...
See the `# SIMD DETECTION` block in `CMakeLists.txt` for details. Most non-distribution sources (`src/common/`, `src/io/`) compile at the platform baseline ISA. The exceptions are the three performance-critical TUs that contain explicit intrinsics: `src/performance/transcendental_kernels.cpp`, `src/calculators/forward_backward_calculator.cpp`, `src/calculators/forward_backward_calculator_mv.cpp`, `src/training/baum_welch_trainer.cpp`, and `src/training/baum_welch_trainer_mv.cpp` — these are listed in `LIBHMM_SIMD_SOURCES` alongside the distribution TUs and receive the full `LIBHMM_BEST_SIMD_FLAGS`.
## Historical context
