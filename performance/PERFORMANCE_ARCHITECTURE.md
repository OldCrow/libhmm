...
- ❌ **`OptimizedMatrix` / `OptimizedVector` / `OptimizedMatrix3D`** — these classes exist in `include/libhmm/linalg/` and contain `std::execution::par_unseq`-gated paths internally, but are **not** on any production data path. The runtime `Matrix`/`Vector` typedefs in `common/common.h` resolve to `BasicMatrix<double>`/`BasicVector<double>`. The Optimized\* classes are exercised only by their own SIMD specialization TUs (`src/common/optimized_vector_simd.cpp`, `src/common/optimized_matrix_simd.cpp`) and by the `test_optimized_*` test files. Treat them as a parallel data layer, not part of the inference hot path.
